...
def create_settings(test_type, vcs_type, server_type="tc"):
    ...
    if test_type == "poll" or test_type == "main":
        settings.AutomationServer.type = server_type
        ...
        elif server_type == "jenkins":
            if test_type == "main":
                settings.JenkinsServerForHostingBuild.build_url = "https://jenkins"
            else:
...
def param(test_type, module, field, vcs_type="*", error_match=None, server_type="tc"):
    # type: (str, str, str, Union[str, List[str]], str) -> None
    global missing_params

    if isinstance(vcs_type, list):
        for vcs in vcs_type:
            param(test_type, module, field, vcs, error_match, server_type)
        return

    if vcs_type == "*":
        param(test_type, module, field, ["p4", "git", "none"], error_match, server_type)
        return
    ...
    missing_params.append(pytest.param(test_type, module, field, vcs_type, error_match, server_type, id=test_id))
...
param("main",   "JenkinsServerForHostingBuild", "build_url",       server_type="jenkins")
param("poll",   "JenkinsServerForTrigger",      "trigger_url",     server_type="jenkins", vcs_type=["p4", "git"], error_match="url for triggering build")
# pylint: enable = bad-whitespace


@parametrize_unset()
@pytest.mark.parametrize("test_type, module, field, vcs_type, error_match, server_type", missing_params)
def test_missing_params(unset, test_type, module, field, vcs_type, error_match, server_type):
    settings = create_settings(test_type, vcs_type, server_type)

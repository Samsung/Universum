...
def stdout_checker(request):
    with mock.patch('_universum.output.terminal_based_output.stdout') as logging_mock:
...
def log_exception_checker(request):
    with mock.patch('_universum.output.terminal_based_output.LocalOutput.log_exception') as logging_mock:
        result = FuzzyCallChecker(logging_mock)
        yield result

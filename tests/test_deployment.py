...
def test_minimal_install(clean_docker_main: UniversumRunner):
    ...
    log = clean_docker_main.environment.assert_unsuccessful_execution(
        f"{ clean_docker_main.environment.python } -m universum")
    ...
    # Run from Git
    clean_docker_main.clean_for_reuse()
...
def test_minimal_install_with_git_only(clean_docker_main_no_p4: UniversumRunner, capsys):
    ...
    # Run from git
    clean_docker_main_no_p4.clean_for_reuse()

...
-I '.*-rw-rw-r--.*' -I '.*-rw-r--r--.*' -I '.*-r--r--r--.*' -I '.*drwx------.*' -I '\s*Created wheel for.*' \
-I '\s*Stored in directory.*' -I 'Requires: .*' -I 'Collecting .*' -I '\s*Downloading .*' \
-I '.* Cherry-picking .*' -I '.* Checking out .*' -I '.*THIS IS A TESTING VERSION.*' -I 'Fetched .* in .*' \
-I '.* passed, .* xfailed in .*' clear_launch.log jenkins_launch.log
exit $?

...
def get_issue_json_format(src_file, diff) -> utils.ReportData:
    # Check block size
    ...
    return utils.ReportData(
        symbol="Uncrustify Code Style issue",
        message=message,
        path=os.path.relpath(src_file, os.getcwd()),
        line=int(diff["line"])
    )
...
class UncrustifyAnalyzer:
    ...
    def parse_files(self):
        files = []
        file_lines = []
        for file_name in self.settings.file_list:
            files.extend(add_files_recursively(file_name))
        for file_list in self.settings.file_list_config:
    ...
    def execute(self):
        ...
        utils.output_report_to_file(issues_loads, self.settings.result_file)
        return 1 if issues_loads else 0
